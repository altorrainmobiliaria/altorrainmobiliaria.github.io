...
| 🧩 **Hojas hijas de `30`** | `docs/31-VERIFICACION-UI.md` · `docs/32-LECCIONES-DOCUMENTALES.md` · `docs/33-LECCIONES-META.md` · `docs/34-DOCTRINA-CODIGO.md` | ❌ | UI en navegador · legal/documental (`LD-NN`) · meta (`M-NN`: dónde falló el cerebro) · 🖥️ doctrina de código. |
| 🔁 **Workflows** | `docs/60-WORKFLOWS.md` | ❌ | Catálogo W-01..W-11; **W-11 = SSoT del flujo fuerte**. |
| 🗂️ **Índice sináptico** | `docs/00-INDICE.md` | ❌ | Mapa §→línea de `99` + capa semántica síntoma→neurona. |
...
- **Cierre (anti "lo documento después")**: no está cerrada hasta verificar — ¿`10` al día? ¿`05` si cambió la salud? ¿decisión → ADR en `99` + fila en `00`? ¿lección → `30`? ¿cache bump (§3.2)? ¿`brain:check` SANO? **¿hubo deliberación? → CRUDO + SÍNTESIS enlazados, o la tarea está INCOMPLETA** (✅ con deliberación no capturada = NO cerrada). Si falta algo, vuelve y hazlo.
