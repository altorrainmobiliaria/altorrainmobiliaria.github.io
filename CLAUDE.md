...
Cada neurona tiene un TOPE BLANDO. Caps, presupuesto de boot y `ssotFacts` viven en `docs/.brain-manifest.json` y los valida `brain:check`: **el manifest es su dueño — NO copies esas cifras aquí** (gate #8). Al acercarse al tope NO engordes: extrae una sub-categoría a `docs/NN-NOMBRE.md` dejando en la madre un **puntero a la hija**. **One-in-one-out**: toda regla nueva en el router DESPLAZA o fusiona otra, y sale del **ROUTER**, no del WIP — el impuesto se paga donde está el peso (§164). Subir el techo NO es cerrar. 🔗 Nada huérfano: neurona que `CLAUDE.md` no conoce = cerebro roto.
