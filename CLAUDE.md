...
| 🧩 **Hojas hijas de `30`** | `docs/31-VERIFICACION-UI.md` · `docs/32-LECCIONES-DOCUMENTALES.md` · `docs/33-LECCIONES-META.md` · `docs/34-DOCTRINA-CODIGO.md` · `docs/35-LECCIONES-PLATAFORMA.md` · `docs/36-LECCIONES-UTILLAJE.md` | ❌ | El nombre dice el tema; lo que NO dice: `32`=`LD-NN` · `33`=`M-NN` (dónde falló el cerebro) · `36`= cuando miente la HERRAMIENTA, no el código. |
...
Cada neurona tiene un TOPE BLANDO. Los caps, el presupuesto de boot y los `ssotFacts` viven en `docs/.brain-manifest.json` y los valida `brain:check`: **el manifest es su dueño — NO copies esas cifras aquí** (gate #8). Al acercarse al tope: NO engordar — extrae una sub-categoría a `docs/NN-NOMBRE.md`, dejando en la madre un **puntero a la hija**. **One-in-one-out**: toda regla nueva en el router DESPLAZA o fusiona una existente, y **sale del ROUTER**, no de la pizarra del WIP — el impuesto se paga donde está el peso (§164). Subir el techo NO es cerrar. 🔗 Nada huérfano: si una neurona existe y `CLAUDE.md` no la conoce, el cerebro está roto.
