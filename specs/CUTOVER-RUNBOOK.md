...
📏 **MEDIDO EL 28-ago corriendo el gate en modo producción de verdad** (§260) — esto es la lista de salida, no una estimación: de las 23 comprobaciones de `verify:build`, **fallan DOS, y las dos son de Daniel**: (1) `PUBLIC_CATALOGO_SOURCE=demo` — publicarías inmuebles que no existen, con precio y barrio; (2) el **RNT** en `/` y `/estancias`. Verde todo lo demás, incluidas las sondas del móvil personal (128 ficheros servidos, legacy dentro), enlaces internos, h1, accesibilidad y vista previa al compartir. *Lo que queda por hacer aquí no es código.*
...
| 5.1b | 🧑 | ⚠️ **Falta la casilla de autorización de datos en `/mi-perfil`** (§263). Hasta §263 esa pantalla **daba por otorgado** el consentimiento sin habérselo preguntado a nadie: era el único sitio del portal donde ese dato nacía, y nacía en `true`. Ya no se fabrica — pero entonces el perfil **no se puede ENVIAR** (guardar sí) hasta que exista la casilla, y la Ley 1581 art. 9 exige poder demostrar la autorización. El patrón ya está escrito en `/alertas` y `/estancias`; es **añadir un control visible**, o sea mockup tuyo. Hoy no bloquea a nadie: no hay ni un perfil creado. |
...
| 5.4 | 🤖 | **Indexable — ya comprobado ANTES del DNS (§260).** Este paso venía **después** del 5.3, o sea con el dominio ya movido: si el build hubiera salido `noindex`, Google no indexa nada y el fallo se descubre cuando ya no hay marcha atrás. Lo cubre `verify:build` en el paso **5.2**, que corre ANTES: *sin noindex residual, robots abierto y sitemap declarado*. Medido el 28-ago construyendo en modo producción: **44 páginas, 6 con `noindex`** — `/gestion` · `/seguridad` · `/design-system` · `/ingresar` · `/mi-perfil` · `/favoritos`—, **las 6 internas y ninguna anunciada en el sitemap**. ⏭ Aquí queda solo la confirmación en vivo, que ya no puede sorprender: `curl -s https://altorrainmobiliaria.co | grep -i noindex` → vacío, y `/robots.txt` sin `Disallow: /`. |
| 5.5 | 🤖 | **Los 65 redirects del sitio viejo — YA MECANIZADO (§259), esto pasó de ser trabajo a ser una lectura.** Antes decía «comprobar los 65 uno por uno» **aquí**, o sea con el DNS ya movido: el peor momento posible para descubrir un 301 roto. Las dos mitades las cubre ahora `verify:enlaces`, que corre en CI **en cada push**: (a) ninguna URL vieja se queda sin regla — 74 cubiertas · (b) **los 28 destinos existen en el build**, o falla nombrando el que no. Las dos probadas EN NEGATIVO. ⏭ Aquí solo queda mirar que el CI del paso 5.2 esté verde. *(Ensayo histórico sobre staging, 2026-08-25: 64/65 + `/index.html` 200 con canonical a `/`, que es consolidación correcta.)* |
