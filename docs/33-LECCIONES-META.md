...
(d) **#3 desync del índice** valida `00 → 99`, pero un `§NN` suelto en PROSA no lo mira nadie: el `10` arrastraba un `§195` que no existe en este repo (0 en `99`, 0 en ambos shards del índice) — un puntero muerto **dentro del boot**, leído en cada sesión.
...
### M-11 — Escribir la lección NO la aplica: si el PENDIENTE no se re-etiqueta, el cerebro la ignora tantas veces como haga falta *(2026-08-21, ADR §94.1 · §96.1 · §97.1)*
**Disparador**: una lección correcta, escrita, indexada y enlazada… que se incumple otra vez a los dos días. **Caso, tres veces en tres días**: [[L-40]] dice «"gateado por el dueño" merece releerse: el gate puede estar en UNA PARTE del alcance». Se escribió el 21-ago al construir el Rango (§94.1). Ese mismo día las **alertas** figuraban «gateadas por clave de Resend y catálogo real» y no lo estaban (§96.1). Horas después la **ficha** figuraba «4 decisiones ANTES de construir» y tres ya traían decisión escrita (§97.1). Tres ítems, tres desbloqueos, una sola lección que nadie aplicó a los otros dos.
