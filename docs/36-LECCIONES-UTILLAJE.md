...
**Reglas**: (1) 🎯 **el disparador es UN `\` O UN `` ` `` EN LA CARGA, no «voy a generar un archivo»**: el 26-ago reincidió ×2 por backslash y ×3 por backtick dentro de `python3 -c "…"` (bash lo ejecuta y deja el hueco VACÍO — caso d). Archivada bajo «generar» no dispara en un parche de una línea ([[M-24]]). Con `\` o `` ` `` → **herramienta de ficheros**; si no, comillas simples en el `-c`. (2) Si no hay remedio: **cadena RAW** (`r'''…'''`) y delimitador entrecomillado (`<<'PY'`, nunca `<<PY`). (3) **Mira los BYTES** al terminar (`cat -A`) — `node --check` valida sintaxis, no intención. (4) ⚠️ En `String.replace`, la CADENA de reemplazo interpreta `$&` y `` $` `` (= «todo lo anterior al match»): usa **función** de reemplazo y el texto entra literal. (6) 🔁 **REINCIDIÓ una 4ª vez el 26-ago**, insertando texto que llevaba `` `main` `` — y el shell se lo comió, dejando la frase coja en un documento sobre fallos silenciosos. La regla estaba bien escrita y aun así falló, porque *«acuérdate de mirar la carga»* depende de acordarse. **Se sustituye por un criterio MECÁNICO, sin juicio**: insertar o sustituir texto en un ANCLA es trabajo de la herramienta de edición —que hace exactamente eso y no pasa por ningún shell—; el intérprete se reserva para lo que la herramienta no puede, que es iterar sobre MUCHOS archivos. Elegir por la FORMA de la operación, y no por el contenido de la carga, quita la decisión de en medio. (5) 🎯 **Prueba que el gate MUERDA**: reintroduce el defecto, comprueba que falla, restaura. Un gate recién escrito que pasa en verde no está probado, está *sin* probar — y esa es la diferencia entre un gate y un adorno ([[M-06]], 4ª forma).
