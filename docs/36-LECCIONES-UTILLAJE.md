...
**Disparador**: la 2ª etapa de un `pipeline()` usaba el ítem original por closure → `s is not defined`; tumbó 4 de 14 y **no 4 cualesquiera**: justo los del veredicto caro (los baratos hacían `return` ANTES de tocarlo). **Causa**: en `pipeline(items, s1, s2)` el callback de s1 es `(item)=>…` pero el de s2 es **`(prevResult, originalItem, index)=>…`**. **Recuperación barata**: `Workflow({scriptPath, resumeFromRunId})` replaya desde caché lo que no cambió. **Reglas**: (1) en etapas ≥2, el ítem sale del **2º parámetro**, nunca del closure; (2) **lee siempre el bloque `<failures>`** — un workflow dice "completed" aunque haya perdido ítems; (3) 🎯 **un bug que vive en UNA rama sesga el resultado**: aquí sobrevivió el 100 % de lo barato y murió el 100 % de lo caro, y un «10/14 ✅» habría sido una conclusión falsa y tranquilizadora. Detalle → §32.9.
