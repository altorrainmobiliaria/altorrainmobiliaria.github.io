...
**Reglas**: (1) 🎯 **el disparador es UN `\` O UN `` ` `` EN LA CARGA, no «voy a generar un archivo»**: el 26-ago reincidió ×2 por backslash y ×3 por backtick dentro de `python3 -c "…"` (bash lo ejecuta y deja el hueco VACÍO — caso d). Archivada bajo «generar» no dispara en un parche de una línea ([[M-24]]). Con `\` o `` ` `` → **herramienta de ficheros**; si no, comillas simples en el `-c`. (2) Si no hay remedio: **cadena RAW** (`r'''…'''`) y delimitador entrecomillado (`<<'PY'`, nunca `<<PY`). (3) **Mira los BYTES** al terminar (`cat -A`) — `node --check` valida sintaxis, no intención. (4) ⚠️ En `String.replace`, la CADENA de reemplazo interpreta `$&` y `` $` `` (= «todo lo anterior al match»): usa **función** de reemplazo y el texto entra literal. (6) 🔁 **CINCO veces el 26-ago**, y las dos últimas merecen contarse porque enseñan cómo se arregla mal una regla. La 4ª: insertando texto con `` `main` `` en un documento sobre fallos silenciosos. Reescribí la regla como criterio mecánico —*«insertar en un ancla es de la herramienta de edición; el intérprete solo para iterar sobre muchos archivos»*— y **reincidí a la vuelta siguiente**, en un `git commit -m "…"`, perdiendo dos identificadores del mensaje. El criterio era correcto y aun así falló: **no cubría el caso**. Un mensaje de commit no es insertar en un ancla ni iterar sobre archivos; es una TERCERA forma —texto que viaja como ARGUMENTO de un comando— y quedaba fuera de las dos ramas que había enumerado.
**Regla, ahora sin ramas que enumerar**: 🎯 **el texto NUNCA viaja como argumento de shell.** A un archivo, con la herramienta de edición; a un comando, por REFERENCIA a un archivo (`git commit -F fichero`, nunca `-m`). El intérprete solo para iterar. *Un criterio mecánico que enumera casos falla en el caso que no enumeraste; el bueno no deja ninguno fuera.* Y el disparador de la recaída fue «este mensaje es corto, no pasa nada» — que es exactamente el juicio que la regla existe para no tener que hacer. (5) 🎯 **Prueba que el gate MUERDA**: reintroduce el defecto, comprueba que falla, restaura. Un gate recién escrito que pasa en verde no está probado, está *sin* probar — y esa es la diferencia entre un gate y un adorno ([[M-06]], 4ª forma).
