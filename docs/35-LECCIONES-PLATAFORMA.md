...
### L-85 — 🤖 Un workflow con `contents: write` es un COMMITTER más: mueve `main` sin que nadie lo pida, y tu rama queda atrás sola *(2026-09-02, sinapsis desde cars)*
**Disparador**: cierras con todo mergeado y más tarde tu `main` local está **detrás** de `origin/main` por un commit que no escribiste — aquí `chore: bump deploy-info.json [skip ci]` (≥8 en el historial), en cars `Auto-generate vehicle pages + bump cache version [skip ci]`. **Causa**: `.github/workflows/bump-version.yml` declara `permissions: contents: write` y termina en `git commit … [skip ci]` + `git push` sobre `main`; el `[skip ci]` y su `if: !contains(…'[skip ci]')` evitan el BUCLE, no la divergencia. En cars es peor: su bot corre además por `cron` cada 4 h y por `repository_dispatch` desde una Cloud Function, así que `main` avanza **con el PC apagado**. **Cómo se manifiesta según la rama de trabajo**: donde se trabaja EN `main` (aquí) el siguiente `push` sale rechazado por non-fast-forward o te obliga a un `pull` que no esperabas; donde hay rama aparte (cars, `dev`) la rama se queda atrás **en silencio**, sin conflicto y sin aviso. **Fix**: «rama de trabajo == `main` == origin» (orden del dueño 2-sep-2026) es una **medida que caduca**, no un estado: se toma con `git fetch` en el turno —`§3.3`: las refs `origin/*` locales son una FOTO, y el heartbeat dice de cuándo (`origin visto hace: N h`)— y en las DOS direcciones: `git rev-list --left-right --count HEAD...origin/main`; derecha > 0 = ventaja del bot → `git merge --ff-only origin/main` (no es deuda tuya), izquierda > 0 = deuda de verdad. **Medido el mismo día**: bersaglio e insema NO tienen bots que commiteen (sus workflows despliegan o auditan). Caso gemelo y detalle del bot: cars `docs/31-LECCIONES-GIT.md` **[[CARS:L-78]]**. Regla enmendada en la skill `sinapsis-cerebros` §3 regla 6 (C4-3).
