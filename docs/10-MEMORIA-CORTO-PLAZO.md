...
| **TODO-22** | **OBRA del catálogo (§54 sellada)**: LECTURA ✅ (§56) · **NÚCLEO de ESCRITURA ✅ (§57**: `construirIndices` rebuild-total idempotente+determinista, omitidas reportadas; vitest 42/42). **Falta la PLOMERÍA**: crear `portal/functions/` + `onWrite` (debounce·query Admin·3 docs `_version+1`·purga `catalogo:{shard}`·seed·scheduled·botón Republicar·retries) + gates emulador (§54.5); **deploy=cutover**. Luego wiring SERP tras flag demo\|live. | 🟡 plomería | §57 |
