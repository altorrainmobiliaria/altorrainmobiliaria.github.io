...
| **TODO-30** | 🗺️ **MapLibre real — IMPLEMENTADO + tiles GENERADOS (§55/§55.8, Opus 2026-07-23)**: isla `altorra-map.ts` + basemap Cartagena `public/basemap/cartagena.pmtiles` (asset estático 3.33 MB, go-pmtiles) + fallback esquemático + card↔pin. Verificado: asset 206+rangos byte-correctos, MapLibre monta+4 marcadores, gates verdes. **Compuerta (a) tiles ✅ CERRADA**. **Falta solo (b): visto bueno VISUAL de Daniel** (WebGL no renderiza en el panel, L-26 → verlo en staging/su Chrome). Luego: wiring forms→`solicitudes`. | 🟢 solo visual | §55.8 |
