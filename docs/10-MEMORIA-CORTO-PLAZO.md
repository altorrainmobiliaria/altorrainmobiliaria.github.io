...
| **TODO-22** | **OBRA del catálogo (§54)**: LECTURA ✅ (§56) · NÚCLEO ✅ (§57) · **PLOMERÍA ✅ (§58**: `portal/functions/` codebase aparte + onWrite/barrido/Republicar + guarda anti-adelantamiento; emulador 33/33). **El índice ya se mantiene solo.** Falta para ver datos REALES: wiring SERP tras flag `demo\|live` · purga HMAC (hoy TTL 10min) · botón en `gestion` · **deploy COORDINADO en el cutover** (rules+functions:portal). | 🟢 falta wiring+deploy | §58 |
