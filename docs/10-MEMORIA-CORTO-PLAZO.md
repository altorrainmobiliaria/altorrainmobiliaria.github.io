...
> **sello tipográfico Cormorant/Hanken vs Cardo/Helvetica-Now · Playfair en hero** (paleta ✅ ratificada §23.8).
...
| **TODO-18** | **Carril D — Diseño**. ✅ **D0 CERRADO** (posicionamiento + paleta + eslogan/misión/visión + voz destilada en skill `catalogo-voz-altorra`). ✅ **DISEÑO ENTREGADO** por Daniel (8 mockups en `portal/design/`). ✅ **D1 SELLADO** (ADR §23 + §23.8): `tokens.css`+`base.css`+`components.css`+fuentes+styleguide, dual-mode, a11y AA. **Paleta OFICIAL ratificada** (§23.8: navy/dorado/plateado #BFC3C9/grises fríos) + **Liquid Glass sutil + card "vitrina"** premium (neumorfismo protagonista). ▶ **SIGUIENTE**: construir la HOME (réplica `ALTORRA Portal.dc.html`). **`[RATIFICAR-DUEÑO]` restante**: SOLO sello tipográfico (Cormorant/Hanken vs Cardo/HelvNow) + Playfair en hero + tú/usted + matrícula/RNT/contacto reales. | 🔄 OPUS | oro/plata mandan; navy discreto; SIN negro |
