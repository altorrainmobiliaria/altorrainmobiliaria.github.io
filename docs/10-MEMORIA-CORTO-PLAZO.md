...
> → imágenes WebP ✅ (§24.10) → SERP `/comprar`+`/arrendar` ✅ (§25) → `/ficha` ✅ (§26) → `/404`+`/publicar` ✅ (§27) → `/estancias` ✅ (§28, reserva funcional) → **invertir/turismo/aliados/gestion/journal/Nosotros/Contacto (siguiente)** → panel-usuario/admin. (MapLibre + datos Firestore + wiring forms→`solicitudes` + pago Wompi Ola 2 = pendientes transversales.)**
