...
| **TODO-47** | 🚪 **ACCESO — construido y desplegado lo que no depende del dueño (§130, §131)**. ✅ candado `loginAttempts` retirado · bitácora REAL · rol en `/gestion` · corte por inactividad · **invitación** (nadie inventa claves) · **suspender** (revoca tokens en el acto) · gate `verify:css` sonda 2. 🔎 El 2FA de cars NO sirve de modelo: decide en una variable del navegador (§130.1). ⏭️ **SOLO falta consola/dueño**: (a) Identity Platform + TOTP — sin vuelta atrás, gratis <50k · (b) contraseña mínima 6→12 · (c) anti-bot apagado · (d) `altorrainmobiliaria.co` sin autorizar (Google roto) · (e) plantilla del correo de invitación. Y la puerta única de 2 pasos (mockup **sin aprobar**). | 🟡 dueño | §131 |
