...
| **TODO-34** | ⭐ **FUNDACIÓN OPERATIVA (§61 mandato · §62 F1 · §63 F2)**. **F1+F2 ✅**: corpus triajeado → `43-OPERACION` (SSoT: veredictos legales verificados, dictámenes de canónica, datos duros para manuales) + bóveda ×8. **SIN contratos vigentes (Daniel, §63.8) — COMPANY arranca de cero. F4 ← SIGUIENTE**: #0 🔴 RUB YA (vencido ~jul-26) + Protocolo de Tesorería · #1 factura/IVA (Siigo+RUT) · #2 KIT de arranque (contrato canónico limpio + tarifario + umbrales — gate para firmar clientes) · #3 matrícula · #4 societario (acuerdo accionistas + SAS vieja) · #5 datos · #6 corta estancia. Gates: L-29 · `.gov.co` · dinero/legal = Daniel (cuestionario → bóveda SINTESIS §6). | 🔄 F4 (Fable) | §63 · `43` |
...
> **▶ 2026-07-24 (sesión Fable, TODO-34)**: F1 ✅ (§62, lóbulo `43-OPERACION`, bóveda ×5); F2 en curso aquí.
> ⚠️ **Para Daniel**: (1) entregar la RESOLUCIÓN de la matrícula de arrendador (el corpus solo trae la
> subsanación) · (2) responder el cuestionario de decisiones (bóveda SINTESIS §6: tarifario, umbrales, cesión
> societaria, contacto registral) · (3) re-exportar `ALTORRA_LIMPIO_FINAL.xlsx` (corrupto).
