...
| **TODO-45** | 🔬 **Deuda de la auditoría #8** (§109, tabla en la bóveda). Por orden: (a) **capa semántica de `00` congelada 24 ADRs** — el ruteo que funciona hoy es la bitácora, así que ESTO va ANTES de podarla; (b) `degrade()` 🟠 cuando un gate hace 0 comparaciones (el ✅ inmerecido: #27 perdona 90 rutas por basename, #16 aprueba «CF 9» contra 11); (c) el **índice revienta en ~4 ADRs**, no en 92 → fila nueva ≤200c bloqueante; (d) umbrales en DÍAS en un repo que corre en COMMITS; (e) `33` por encima del tope; (f) **`30` a 1 línea del cap** (230/231) con chars de sobra: ejes descalibrados → shard, no techo ([[M-05]]). | 🔴 | §109 |
