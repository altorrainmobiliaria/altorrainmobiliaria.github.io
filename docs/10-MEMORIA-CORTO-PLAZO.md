...
| **TODO-32** | 🧠 **CEREBRO v2 ×4 — PROPUESTA ENTREGADA, espera OK de Daniel** (2026-07-20). SSoT = bóveda `2026-07-20-cerebro-v2-sintesis-propuesta.md` (comité ×3 + consejo Antigravity debatidos). Núcleo: regla de admisión de maquinaria (bloqueante-o-hook + borra lo que reemplaza) · F0 restar+blindar (kill-list kernel + `git bundle` ×5 → OneDrive PROBADO) · F1 kernel único (`brain-private/kernel/` + hash-gate bloqueante) · F2 heartbeat local→sidecar + consolidación-en-frío + `brain:archive` · F3 resonancia empujada por gate #14 + banner. ~5-6 sesiones Opus. Al aprobar: F0 abre con ADR. Absorbe TODO-31a (offsite→F0) y TODO-23 (kill-list→F0). | ⏸️ Daniel | §49 · bóveda |
