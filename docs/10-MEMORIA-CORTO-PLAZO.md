...
> **sello tipográfico Cormorant/Hanken vs Cardo/Helvetica-Now · paleta entregada vs visión · Playfair en hero**.
...
| **TODO-18** | **Carril D — Diseño**. ✅ **D0 CERRADO** (posicionamiento + paleta + eslogan/misión/visión + voz destilada en skill `catalogo-voz-altorra`). ✅ **DISEÑO ENTREGADO** por Daniel (8 mockups en `portal/design/`). ✅ **D1 SELLADO** (ADR §23): `tokens.css`+`base.css`+`components.css`+fuentes+styleguide `/design-system`, dual-mode, a11y AA verificado en vivo. ▶ **SIGUIENTE**: construir la HOME (réplica `ALTORRA Portal.dc.html`). **`[RATIFICAR-DUEÑO]`**: sello tipográfico (Cormorant/Hanken vs Cardo/HelvNow), paleta entregada vs visión, Playfair, ajuste a11y de la firma, + tú/usted, matrícula/RNT/contacto reales. | 🔄 OPUS | oro/plata mandan; navy discreto; SIN negro |
