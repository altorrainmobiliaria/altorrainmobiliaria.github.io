...
| **TODO-32** | 🧠 **CEREBRO v2 ×4 — APROBADO por Daniel (2026-07-20)**. SSoT = bóveda `2026-07-20-cerebro-v2-sintesis-propuesta.md` + regla de admisión de maquinaria. **F0 ✅ (§50)** kill-list + offsite probado · **F1 ✅ (§51)**: canónico en `brain-private/kernel/` + `brain:pull` + gate #0 BLOQUEANTE — **×4 en v1.4.1, fix propagado <2 min**. Deuda visible en hermanos (cars 7 · bersaglio 8 problemas + auditorías vencidas) = SUS carriles. `core.hooksPath` diferido a F2/F3. **🏁 v2 NÚCLEO COMPLETO ✅** (F0 §50 · F1 §51 · F2 §52 · F3 §53, kernel **v1.6.0** ×4): las 4 clases de dolor probadas quedaron imposibles-por-diseño o empujadas-por-gate. **Restos vivos**: (a) F2-s2 — cablear hook SessionStart en los 3 hermanos (kernelFiles+settings; carril de cada repo, la instrucción vive en la skill) · (b) **PODA REAL de doctrina** cuando la métrica del banner marque >30% dos meses (criterio de salida — hoy 52% 🔴, mes 1 de medición) · (c) TODO-31 c/d (runbook recuperación cuenta · shard `30`). Mantenimiento mensual: el banner avisa TOCA → Daniel dice "haz el mantenimiento mensual" → skill `mantenimiento-general`. | 🟡 restos a/b/c | §53 · bóveda |
| **TODO-31** | 🛡️ **Resiliencia + GC mayor**: (a) ✅ **offsite DEFAULT (F0 §50)** — bundles ×5 → OneDrive + restore PROBADO + sello `lastOffsiteBackup` (espejo en 2ª cuenta git = mejora OPCIONAL de Daniel) · (b/b2) ✅ canario + guardián bóveda (§49-bis) · (c) runbook recuperación de cuenta GitHub en `50-CONFIG-INFRA` (2FA/códigos — verifica Daniel) · (d) GC de `30` (96%): shard L-22/L-26/L-28 → hoja `31-VERIFICACION-UI` + fusión L-04/L-09 — **hacerlo en la 1ª resonancia** (plan en tabla §49 de la bóveda). | 🟡 c/d | §49 · §50 |

---

## 📝 Bitácora (efímera)

> **▶ RELEVO A SESIÓN FRESCA (2026-07-23, cierre del arco cerebro — Daniel: "documenta todo, inicio fresca")**:
> (1) **Arco COMPLETO en 2 sesiones**: Auditoría Nivel-2 #4 (§49, SANO + TODO-30 blindado + M-03) → propuesta v2
> (comité ×3 + consejo Antigravity, síntesis en bóveda) → Daniel APROBÓ TODO → **F0-F3 ejecutadas (§50-§53)**:
> kernel **v1.6.0 canónico ×4** vía `brain:pull` + gate #0 · heartbeat (el `05` ya no contiene derivables) ·
> consolidación-en-frío · `brain:archive` · banner en cristiano · skill `mantenimiento-general` · offsite probado.
> (2) **SIGUIENTE RECOMENDADO = TODO-30 MapLibre (Opus, EN FRESCO)** — la fila está BLINDADA (tiles sellados
> Protomaps · solo ficha+serp · criterio de preservación); el negocio necesita cerrar obra. Carril cerebro (menor):
> restos TODO-32 (hooks hermanos) · TODO-31 c/d · TODO-23 K-restantes → resonancias.
> (3) ⚠️ **HUMO a D+5**: seguro agotada — verificar Ads Manager (factura/CPM → fontanería §4b, TODO-29) cuando haya navegador.
> (4) ⛔ NO tocar: datos Firestore (TODO-22 Fable) · diseño sellado · deuda de carriles hermanos (cars 9 · bersaglio 8 — SUS sesiones la curan, sus gates ya la muestran).
> (5) 🧠 Regla nueva de oro del kernel: NUNCA editar `scripts/brain-*.mjs` localmente — canónico → VERSION → pull ×4.
