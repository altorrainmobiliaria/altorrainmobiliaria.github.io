...
| **TODO-31** | 🛡️ **Resiliencia + GC mayor**: (a) ✅ offsite DEFAULT (F0 §50) — bundles ×5 → OneDrive + restore PROBADO (espejo 2ª cuenta git = mejora OPCIONAL de Daniel) · (b/b2) ✅ canario + guardián bóveda (§49-bis) · (c) ✅ **runbook recuperación cuenta GitHub/CF ESCRITO** (`50 §Runbook` 2026-07-23) — ⚠️ falta SOLO que Daniel descargue/verifique los recovery codes (GitHub + Cloudflare) · (d) ✅ **GC de `30` EJECUTADO** (2026-07-23): shard L-22/L-26/L-28 → hoja `31-VERIFICACION-UI` + fusión L-04→L-09 (30: 97%→~75% de cap). | 🟡 solo (c) verif. Daniel | §49 · §50 |
...
> (4) **SIGUE = TODO-30 MapLibre (sesión Opus EN FRESCO — fila blindada)**. Carril Fable siguiente: **TODO-22**
> (capa de datos §22 `[REVISAR-FABLE]` + catálogo SSG vs doc-índice; W-11 usando §0b). Resonancias: TODO-23 K-rest · hooks hermanos.
