...
| **TODO-22** | **OBRA del catálogo COMPLETA en código (§54)**: LECTURA ✅ §56 · NÚCLEO ✅ §57 · PLOMERÍA ✅ §58 · **SERP cableado ✅ §59** (isla tras flag; demo intacto). **Para que el visitante vea datos reales falta SOLO: (1) deploy COORDINADO en el cutover (rules + `functions:portal`) · (2) `PUBLIC_CATALOGO_SOURCE=live` · (3) sembrar propiedades.** Opcionales: purga HMAC (hoy TTL 10min) · botón Republicar en `gestion` · ficha dinámica por id · paginación. | 🟢 solo cutover | §59 |
