...
| **TODO-32** | 🧠 **CEREBRO v2 ×4 — APROBADO por Daniel (2026-07-20)**. SSoT = bóveda `2026-07-20-cerebro-v2-sintesis-propuesta.md` + regla de admisión de maquinaria. **F0 ✅ (§50)** kill-list + offsite probado · **F1 ✅ (§51)**: canónico en `brain-private/kernel/` + `brain:pull` + gate #0 BLOQUEANTE — **×4 en v1.4.1, fix propagado <2 min**. Deuda visible en hermanos (cars 7 · bersaglio 8 problemas + auditorías vencidas) = SUS carriles. `core.hooksPath` diferido a F2/F3. **F2 piloto ✅ (§52)**: 💓 heartbeat (sidecar + eco; 05 sin derivables; costo 52% 🔴 midiéndose solo) · 🧊 consolidación-en-frío · 📦 `brain:archive` · kernel **v1.5.1** ×4 (punto ciego gate #0 curado). **Pendiente F2-s2**: cablear hook SessionStart en los 3 hermanos (carril de cada repo o junto a F3). **Siguiente → F3** (1 sesión): gate #14 escala info→warn→FAIL con gracia (maxDays+7) + banner cristiano (3 líneas: costo · backup · toca-resonancia) + skill `mantenimiento-general` (ejecutor: rotación Nivel-2 con sonda de contradicción + banco retrieval + poda one-in-one-out + bundle automatizado). | 🔄 F3 next | §52 · bóveda |
