...
| **TODO-32** | 🧠 **CEREBRO v2 ×4 — APROBADO por Daniel (2026-07-20)**. SSoT = bóveda `2026-07-20-cerebro-v2-sintesis-propuesta.md` + regla de admisión de maquinaria. **F0 ✅ (§50)** kill-list + offsite probado · **F1 ✅ (§51)**: canónico en `brain-private/kernel/` + `brain:pull` + gate #0 BLOQUEANTE — **×4 en v1.4.1, fix propagado <2 min**. Deuda visible en hermanos (cars 7 · bersaglio 8 problemas + auditorías vencidas) = SUS carriles. `core.hooksPath` diferido a F2/F3. **Siguiente → F2** (2 sesiones, piloto aquí → pull ×4): heartbeat `--fast` solo-local → sidecar gitignored ecoado al boot + el `05` PIERDE los campos derivables (atómico) + consolidación-en-frío (SessionStart presenta ADRs pendientes como 1ª tarea) + `brain:archive` (fontanería mecánica del cierre) + métrica de costo por paths en banner. Luego F3 (resonancia por gate #14 + banner + skill `mantenimiento-general`). | 🔄 F2 next | §51 · bóveda |
