...
> → imágenes WebP ✅ (§24.10) → SERP `/comprar`+`/arrendar` ✅ (§25) → `/ficha` ✅ (§26) → `/404`+`/publicar` ✅ (§27) → `/estancias` ✅ (§28) → `/turismo` ✅ (§29) = **TODAS las páginas públicas mockup-backed LIVE.** SIGUIENTE: **Gestion (panel admin, único mockup restante)** + sin-mockup (invertir/aliados/journal/Nosotros/Contacto — requieren aprobación §3.2) + transversales (MapLibre, datos Firestore, wiring forms→`solicitudes`, Wompi Ola 2). **Maintenance: auditoría Nivel-2 (TODO-19, 16 ADRs).**
