...
| **TODO-34** | ⭐ **FUNDACIÓN OPERATIVA (§61-§65)**. **F1+F2+F4 (w1+w2) ✅**: sistema documental **00-23 COMPLETO** en bóveda `entregables-fundacion/` (kit 14 + MANUAL MAESTRO 10 caps + anexos 16-21 + recomendaciones-mercado 22 + encargo-pagaré 23; md+docx, todo auditado adversarialmente). Tarifas comerciales/ACM/cancelación **adoptadas-vetables con fuentes** (delegación Daniel). **Pelota**: (a) Daniel ejecuta `00-LEEME` (FE-DIAN gratuita YA · cuenta escudo · cotizar póliza ×3 · acuse RUB · resolución matrícula al final) + corre el **prompt consejo-externo del manual** (bóveda) · (b) abogado: doc 23 + agenda cap 8 · (c) Claude: integrar la crítica de Gemini · skills F3 cuando ruede · cerebro dual Code+Chat. | ⏸️ Daniel + Gemini | §65 · `43` |
