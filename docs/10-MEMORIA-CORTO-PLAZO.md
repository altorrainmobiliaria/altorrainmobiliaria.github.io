...
| **TODO-42** | 🔑 **Claim de staff — ✅ DESPLEGADO 2026-08-24** (§124): `claimsStaffSync` + `sincronizarClaimsV2` vivas, `functions:list` da 9. ⏭️ **Falta SOLO el clic de Daniel**: admin.html → Usuarios → «Sincronizar permisos», y luego abrir `/gestion`. Es lo único que 🤖 no puede hacer (la callable exige token de super_admin). Hasta ese clic, la fase 2 (reglas) NO se despliega: sin claims nadie sería staff. | 🟡 clic | §124 |
