...
- `~/.claude/settings.json` (user-level) → habilita **21 plugins** de `claude-plugins-official`, no ~190: **podado el 2026-09-03** tras el `spawn ENAMETOOLONG` ([[L-86]]). Las otras 169 entradas siguen listadas en `false` (reactivar = cambiar el booleano, no reinstalar). Vivos: `superpowers` · `firebase` `cloudflare` `github` `resend` `mapbox` · `chrome-devtools-mcp` `playwright` `firecrawl` `context7` · `commit-commands` `code-review` `pr-review-toolkit` `code-simplifier` `feature-dev` `session-report` · `skill-creator` `plugin-dev` `claude-md-management` `claude-code-setup` · `frontend-design`.
