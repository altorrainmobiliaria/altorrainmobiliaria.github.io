...
> 📊 <!--CIFRA-CEREBRO--> **Skills GOBERNADAS que este repo consume: 46** — las declaradas en `skillFiles` de `docs/.brain-manifest.json` y repartidas por `npm run brain:pull` desde el canon de la bóveda. La cuenta la comprueba el gate #29; las ~114 carpetas de `skills/` de abajo son el catálogo histórico, que es OTRA cifra.
...
- `~/.claude/settings.json` (user-level) → habilita **27 plugins** de `claude-plugins-official`, no ~190: **podado el 2026-09-03** tras el `spawn ENAMETOOLONG` ([[L-86]]). Las otras **163** entradas siguen listadas en `false` (reactivar = cambiar el booleano, no reinstalar). ⚠️ **RE-MEDIDO el 2026-09-04 (auditoría #19)**: eran 21 al podar y hoy son 27 — volvieron `telegram` `vercel` `mercadopago` `stripe` `supabase` `nvidia-skills`, y el margen de línea de comandos que se publicó con 21 (**6.729c de 32.767**) ya NO describe este conjunto: re-medirlo antes de citarlo. Vivos: `superpowers` · `firebase` `cloudflare` `github` `resend` `mapbox` `supabase` `vercel` `stripe` `mercadopago` `nvidia-skills` `telegram` · `chrome-devtools-mcp` `playwright` `firecrawl` `context7` · `commit-commands` `code-review` `pr-review-toolkit` `code-simplifier` `feature-dev` `session-report` · `skill-creator` `plugin-dev` `claude-md-management` `claude-code-setup` · `frontend-design`.
...
| `optimizacion-rendimiento-web` | **Playbook agéntico de rendimiento web** (medir→priorizar→implementar réplica-EXACTA→verificar LIVE→iterar por página): self-host fuentes+`unicode-range` · dieta JS · critical-path CSS · imágenes/LCP · SW/cache · diferir terceros. ORQUESTA las hermanas (SEO/imágenes/`validacion-live-chrome`/`caza-bugs`), no las duplica. VIVA (cada página enriquece el catálogo) + script `fetch-fonts.mjs`. Portable ×4. Origen CARS §297 (self-host fonts → 0 req a Google, verif live). | ✅ repo+user |
