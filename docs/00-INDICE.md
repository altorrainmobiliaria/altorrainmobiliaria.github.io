...
| §103 | 🔀 **Dos escritores, un almacén, dos modelos**: `admin.html` y el portal comparten `propiedades` y escriben esquemas incompatibles; el `as Propiedad` a ciegas los deja pasar el filtro y caerse después → **índice vacío, cero errores**, y omitidas como `sin-precio` (diagnóstico falso). Motivo propio `esquema-legacy` + mismo gate en la ficha (el id legacy se teclea a mano) + `omitidasPorMotivo`. **TODO-44 pasa a requisito del cutover.** | 3162 |
| §104 | ⚖️ **El gate del RNT protegía la ficha y dejaba pasar la CARD**: `publicable()` (alojamiento sin RNT = publicidad ilegal, B3) solo lo llamaba la ruta de ficha; el índice no → la card habría salido en `/estancias` enlazando a un 404. Se muda al MODELO (invariante legal, no de vista) + motivo propio `sin-rnt`. El fixture de la suite modelaba un anuncio ilegal. Cazado por [[L-45]](e). | 3219 |
| §105 | ⚖️ **«Avalúo» — desbloqueado leyendo la regla, no su resumen**: `42-LEGAL §9` no veta la palabra, veta **atribuirse la actividad regulada** (Ley 1673/RAA). `/publicar` ofrecía «avalúo gratis» mientras su hermana `/rango-altorra` hacía lo mismo bien. Corte por **quién produce el número**: nuestra estimación → «valoración» + disclaimer; línea de servicio → hecho de Daniel, no se toca. | 3266 |
| §106 | 🔎 **Recon del alta + 2 defectos vivos**: `geo.ciudad` vacía → arriendo anunciado en Cartagena SIN matrícula (Ley 820 art.31), y `imagenPortada:''` → sin card y sin imagen al compartir. Dueño único (`ciudadDe`/`portadaDe`). El recon TUMBÓ 4 premisas de TODO-44: **las fotos no tienen camino** (R2 sin código, `verify:data` veta storage), `_version` no ata al super_admin, `captaciones` es `write:false`, y el índice no está desplegado. | 3323 |
...
| §108 | 🏗️ **El alta de propiedades, todo menos la pantalla** (TODO-44): `problemasParaPublicar()` LLAMA a los predicados del lector — la respuesta a §103, con test en las dos direcciones. **Dos parsers numéricos**: en un precio el punto son miles, en una coordenada es el decimal (uno solo pondría el inmueble en la latitud 10399). Transacción con `tx.get` porque `_version` NO ata al super_admin. Falta la pantalla: gatea el mockup. | 3443 |
