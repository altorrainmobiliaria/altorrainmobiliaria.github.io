...
| §56 | **§54 obra — catálogo camino de LECTURA** (índice denormalizado `indices/catalogo-{shard}`): `catalogo.get()` + rules `indices` + ruta `/api/catalogo/*.json` + tests (33 unit + 20 rules). Ruta DORMIDA (SERP sigue demo) hasta la mitad de ESCRITURA (Function) + cutover. ⟦OPUS-4.8⟧ | 914 |
