...
| §48 | **TODO-27 CERRADO — HOME+PUBLICAR FIEL** (re-audit 4/4). 🏁 **6 páginas fieles** (balance de la saga §43-§48). ⟦OPUS-4.8⟧ | 806 |
| §47 | **TODO-27: SERP FIEL** (`[operacion].astro`): /comprar=4 venta + /arrendar=1 arriendo REALES (sin listings inventados) + filtros/sombra/3ª vía. Re-audit 8/8. ⟦OPUS-4.8⟧ | 767 |
...
| §42 | **HUMO MONTADA y ENCENDIDA** (campaña `120250036063330588` Leads+CTWA $4.000/día Cartagena+40km; Meta aprobó). Gotcha página default de CARS → L-32. ⟦FABLE-5⟧ | 595 |
...
| §39 | **Constancias ×3 COMPLETAS + pauta de humo + cierre** (TODO-20 cerrado ×3; humo ~COP 5k → playbook §4b; WhatsApp Web "Sin conexión" = abrir app en el teléfono). ⟦FABLE-5⟧ | 516 |
...
| §66 | **CONTRATOS 03/04 BLINDADOS: pagaré RETIRADO · ALTORRA arrienda EN NOMBRE PROPIO (C.Co. 1262) · dictámenes propios**. Comité ×6 + consejo externo ×2 rondas: 136 hallazgos → 126 sobrevivientes (2 críticos), 14 descartados con razón. La DÉCIMA SEGUNDA del 04 pasó de pagaré a **PÓLIZA** (vacío real); prima mensual, amparos con casillas y ventana de 60 días; fondo de reserva; fallecimiento reescrito (2189/2194/1634); causales del art. 22 completadas (cambio de destinación = alojamiento por días); "indemnización" → subsistencia del canon; pena e intereses por causas separadas. Normas leídas en fuente oficial. ⟦OPUS-5⟧ | 1016 |
| §65 | **TODO-34 F4-w2 ✅: MANUAL MAESTRO (10 caps) + mercado → sistema documental 00-23 COMPLETO**. Tarifas comerciales adoptadas-vetables con costumbre certificada · FE gratuita DIAN soporta mandato nativo · prompt consejo externo del manual listo. ⟦FABLE-5⟧ | 1006 |
| §64 | **TODO-34 F4-w1 ✅: KIT FUNDACIONAL 14 docs + Word + `00-LEEME`** (10 redactores + 5 auditores → 37 fixes aplicados). Patrón redactor→auditor-adversarial→aplicador PROBADO. ⟦FABLE-5⟧ | 996 |
| §63 | **TODO-34 F2 ✅: verificación legal 6 frentes .gov.co + Gemini integrado**. RUB vencido→YA · CE SAGRILAFT derogada 02-jul-26 · 5 prácticas ILEGALES · canon-neto legal (4 formalidades de mandato) · §63.8 cero contratos vigentes → F4 reordenado. ⟦FABLE-5⟧ | 985 |
| §62 | **TODO-34 F1 ✅: triaje del corpus (143, 9 lectores) → nace `43-OPERACION`**: doble NIT · matrícula sin resolución · gerente sin mayoría · KYC→FONPRECON · tarifario inexistente. ⟦FABLE-5⟧ | 975 |
| §61 | ⭐ **PIVOTE DE MISIÓN: Fundación Operativa (TODO-34)** — armar la inmobiliaria completa con datos reales (Claude=abogado+empleados · cerebro dual Code+Chat). Corpus REAL: 83 docs + operación VIVA (2 administrados); gaps: corta estancia · notaría · inquilino · contable. Plan F1-F4. ⟦FABLE-5⟧ | 965 |
...
| §56 | **§54 obra — catálogo camino de LECTURA** (índice denormalizado `indices/catalogo-{shard}`): `catalogo.get()` + rules `indices` + ruta `/api/catalogo/*.json` + tests (33 unit + 20 rules). Ruta DORMIDA (SERP sigue demo) hasta la mitad de ESCRITURA (Function) + cutover. ⟦OPUS-4.8⟧ | 916 |
| §55 | **TODO-30: mapa REAL MapLibre v6 + Protomaps** en ficha+SERP (isla · marcadores navy/oro · card↔pin · degradación esquemática). **§55.8: tiles Cartagena GENERADOS (go-pmtiles) + empacados como ASSET ESTÁTICO** (`public/basemap/`, 3.33 MB, §3.7 refina el sello R2) → falta solo visto bueno visual. BUG: `locals.runtime.env` removido v6 → `cloudflare:workers` (L-33). ⟦OPUS-4.8⟧ | 902 |
