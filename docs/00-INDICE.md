...
| §22 | **Ola 0.7 (parte 3/3): capa de datos `client.ts`** (lecturas públicas Firestore REST + Workers Caching, edge-safe). Decisión Fuerte OD1 `[REVISAR-FABLE]`: comité ×3 cazó BLOCKER de decode (mapa/array vacío) + anti-traversal + memo footgun + TTL por-PoP. Gate empírico: tsc + vitest 26/26 + astro build + verify:data + T6 rules 15/15 (emulador; confirma inexistente→403). | 181 |
...
| §66 | **CONTRATOS 03/04 BLINDADOS**: pagaré RETIRADO · ALTORRA arrienda EN NOMBRE PROPIO (C.Co. 1262) · dictámenes propios en vez de "pendiente de abogado". Comité ×6 + consejo externo ×2: 136 hallazgos → 126, 14 descartados con razón. La DÉCIMA SEGUNDA del 04 pasó de pagaré a PÓLIZA; fondo de reserva; fallecimiento reescrito; causales del art. 22 completadas; pena e intereses por causas separadas. | 1016 |
...
| §69 | **Auditoría Nivel-2 #5** (la disparó el GATE de pre-commit, no un encargo): retrieval frío **5/6** y la hoja nueva 32 enruta limpio · 2 fallos de ruteo CURADOS (faltaba la fila del ARRENDADOR; dev-sí/prod-no citaba L-33 en vez de L-34) · **REINCIDENTE**: TODO-31 perdió su fila y A-01/A-03 llevaban 11 días invisibles · costo 49%→55% · **CRÍTICO: el kit legal no tiene linter** → TODO-35. Masa-neta −37c. | 1191 |
| §70 | **B-03 APLICADA (14 críticos) + los ESTATUTOS entran al cerebro**: la auditoría revisó el kit contra el kit y nunca abrió los estatutos → de las 4 decisiones que se le subieron al dueño, **3 ya estaban resueltas** (precio por peritos art. 8º e · supramayoría 70% art. 13º · arbitramento art. 24º). Daniel: el **doc 13 se RETIRA**. LD-05 nueva + LD-02 reincidente. | 1247 |
| §71 | **Los 23 ALTOS de B-03 aplicados + los 2 primeros gates documento↔documento**: 12 planificadores + 1 revisor de colisiones (13 ag.). **21 vigentes · 1 ya resuelto · 12 de 23 degradados**. El revisor cazó 2 choques de escritura (uno perdía un edit en silencio) y que los planes 07/08 iban a escribir una **falsedad jurídica** sobre la versión de la Política. Formato C atribuía a ALTORRA un biométrico de la aseguradora. LD-06 nueva. | 1306 |
| §72 | **Heartbeat+handoff a los 3 hermanos (TODO-32a ✅)**: sin él, el estado derivable se copia a mano y se desincroniza — el 05 de cars declaraba una cache de 8 días atrás. insemastereo no tenía NINGÚN SessionStart. Re-midió el SPOF: 2 de 3 pendientes eran falsos. Destapó que el boot-gate solo existe en inmobiliaria (cars +4.1k, bersaglio +11.6k) y que brain-kit está congelado pre-v1.6.0. | 1372 |
| §73 | **Auditoría de insemastereo aplicada + bersaglio destilado**: 36 hallazgos → 14 en un cerebro que el linter daba SANO; 5 sondas cazaron el mismo `05` mintiendo sobre git. El heartbeat generaba la verdad y ningún nodo la enrutaba (§72 estaba a medias). bersaglio llevaba tiempo SIN poder commitear (2.6k sobre tope) → 30k→26.7k. 6 chequeos de kernel a TODO-23. | 1422 |
| §74 | **cars destilado (35.9k→33.9k) + `brain-kit` descongelado**: la tabla de topes de §G.5 tapaba 2 hojas SIN cap — una tenía 27k y ningún gate la miraba (→ chequeo #23: el manifest calla y el linter solo vigila lo declarado). La plantilla de instalación llevaba desde el 18-jul sin heartbeat: un cerebro nuevo nacía a medias. NO se instaló el boot-gate: mover el techo para alcanzarlo es la trampa que el gate impide. | 1477 |
| §75 | **kernel v1.7.0 — chequeos #17 y #23**: el kernel nunca miraba el git del repo que audita (por eso un 05 mintió 42 días con 16 gates en verde), ni vigilaba las neuronas que el manifest no declara (44 sin techo ×4). Se verificaron ENCENDIDOS: la v1 de #17 no cazaba el caso real y la v2 acusaba a un inocente. El propio linter me cazó 2 errores en el mismo turno. | 1520 |
...
| §82 | **TODO-35 CERRADO: el kit de firma estrena sus 6 gates cruzados** (cifras vs doc 02 · remisiones a docs RETIRADOS · identidad/canales · anclas de la figura del ARRENDADOR). Cierra el crítico N5-05. Las 2 excepciones que solo salen probando: la cita que DEROGA y la cifra AJENA ([[LD-07]]). Cazó 2 defectos reales (el 00-LEEME y el manual mandaban al doc 13 retirado). Nuevo `-SoloGates`. | 1792 |
