...
| §89 | 🏗️ **`/ingresar` y `/favoritos`** — las 2 pantallas que el header enlazaba a un 404. Los favoritos van en **localStorage, NO detrás del login** (el mockup dice «ingresar para SINCRONIZARLOS»): funcionan hoy para todos. Cards por CLON del `<template>` (nunca innerHTML). **Crear cuenta NO se abre**: falta publicar la Política de Datos (Ley 1581 art. 9). | 2340 |
| §90 | 🔬 **Auditoría Nivel-2 #7** (encargo de Daniel; 4 drills fríos + sondas directas): **ruteo SANO 4/4, pero 2/4 responderían MAL** por frescura. [[M-10]]: tres gates verdes cubrían MEDIA promesa (#27 sin el verbo `creas` · #5 valida existencia no corrección · memoria del harness SIN gate ⇒ 2 reincidentes). **N7-00**: la campaña de humo puede ser ZOMBIE — el `05` la da apagada, su SSoT la da ACTIVA. GC −87c. | 2418 |
| §91 | 🔎 **SEO técnico de OLA 1 + el interruptor que desindexaba en silencio**: `PUBLIC_SITE_ENV` se leía en 2 sitios y no se declaraba en NINGUNO ⇒ todo build salía `noindex` — el del cutover también. Mapa de **68 URLs** → 301 (+9 exentas), `robots`/`sitemap` conscientes del entorno, y candado en `verify:build` #6 que FALLA si el noindex sobrevive en prod. El archivo de verificación de GSC daba 404: duplicado. | 2464 |
| §92 | 🗺️ **13 landings de zona** (ítem 4): destinos definitivos de los barrios del legacy; redirects y sitemap **derivados de `ZONAS`** para que no puedan desincronizarse. **CERO datos cuantitativos** (el portal ya se quemó con listings fabricados); lo del dueño en `PENDIENTE_DUENO`, sin renderizar. Al pasar: el portal **no tenía ni un JSON-LD** (el legacy tenía 43) ni `canonical` en ninguna página. | 2520 |
| §93 | 💰 **`/precios`** (ítem 7, «el diferenciador gratis»): tarifas públicas con las cifras **SELLADAS** de `43`, no las del MEGA-PLAN, que están SUPERADAS (decía 2-3% y 50-100% del canon; lo real es 3% y 1/2/3 cánones por duración). Lo no decidido se dice. `FAQPage` JSON-LD. ⚠️ §93.6: el footer dice «Avalúo gratis» y B13 lo prohíbe — dos fuentes del cerebro en contradicción legal. | 2560 |
...
| §95 | 🔖 **JSON-LD del negocio** en todas las rutas (`RealEstateAgent`, desde `BaseLayout`): cierra el hueco que el legacy sí cubría en 43 páginas. Lo valioso son las 2 ausencias deliberadas: **sin `streetAddress`** (la dirección comercial aún falta) y **sin `aggregateRating`** (no hay reseñas propias; inventarlas la sanciona la SIC). El markup de inmueble espera al catálogo real. | 2640 |
| §88 | 🏗️ **El formulario de captación deja de perder los leads**: `/publicar` era demo (`TODO: POST real`). Endpoint `/api/solicitud` (funciona SIN JS) → `solicitudes` con el contrato del legacy; se pudo HOY porque las reglas VIVAS ya dicen `allow create: if true`. Probando END-TO-END salió que **el correo de avisos está ROTO en producción** (credenciales Gmail). [[L-33]] reincidente. | 2265 |
| §87 | **Leves lotes 2-3 (28/92)**: el grupo "retirados" NO era moot — 5 de 7 eran de documentos vivos, y en el doc 13 la **no-competencia** faltaba en su lista de riesgos asumidos. En los documentos de FIRMA, 7/7 vivos: la **retención se repartía por mitades** como si fuera costa de escritura, y no había salida para el negocio que muere ya celebrado (**FASE G**). 2º remedio dañino. | 2199 |
