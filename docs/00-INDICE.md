...
| §32 | **Fidelidad + ELEVACIÓN de diseño**: el portal DIFIERE de los mockups (Daniel lo cazó → L-24; mapa 7 páginas §32.2). Mandato: fusión neu+skeu+glass+liquid, app-like, premium sin choque (§32.3). **Header v3** (glass full-bleed + emblema oficial Canva + íconos Lucide/Simple embebidos [astro-icon rompe Workers, L-23] + auto-hide) + **Hero** carrusel. **§32.8 `#arriendo`→LISTA + filtro administración** (corregido 07-16). **§32.9 AUDITORÍA de la home**: mapa definitivo de las **17 secciones** (10 AUSENTES · 3 DIVERGENTES —`#cerca` debe ser **buscador+MAPA**, no grilla— · 4 FIELES); `#destacadas` `disenoPropio:false` ⇒ `PropertyCard` NO es el villano; **`.arail` = riel compartido ×4** (venta/estancias/valoradas/proyectos) ⇒ 1 `Rail`+1 `LuCard`. **§32.10 base de riel** `.alt-rail`+`LuCard` (reusable ×4) + **#venta** fiel; corrige un BUG del mockup (sus flechas de carrusel no funcionan: botones en un `[data-railwrap]` hermano del riel) → L-26 (renderer del panel CONGELADO: rAF=0, sin eventos `scroll`, sin captura). **§32.11-.13 los 4 CARRUSELES completos** (venta·estancias·valoradas·proyectos): el gate pagó ×3 — **5 cards y NINGUNA intercambiable** (PropertyCard·LuCard·StayCard·RankCard·ProjectCard); lo único compartido es `.alt-rail` (1 JS genérico para los 4). Ritmo de fondos neu↔mist (`.home-band--neu`). 0 off-palette en TODA la home. **§32.14-.18 HOME COMPLETA (17/17 secciones del mockup, en su orden)**: 2 splits + 2 mosaicos + CTA + muro IG + **#cerca corregida** (era grilla con titular reescrito y 3 propiedades INVENTADAS → ahora buscador + MAPA ilustrado; MapLibre sigue siendo follow-up). **Hallazgo §32.15: el LIENZO de la home es NEU** (`body{background:#eaf0f7}` en el mockup), no blanco — sin eso el neumorfismo no tiene relieve. Ritmo neu↔mist↔navy. 6 cards, ninguna intercambiable. 3 correcciones de disciplina al mockup (scrims casi negros → navy) + handle real (@altorra.co era falso). **L-27**: un grep da la HOJA, no la RAMA (asumí `SITE.contacto`, era `contact`). 0 off-palette en TODA la home. **§32.20-.22**: Estancias(+Reseñas)/Publicar(+franja)/SERP(+interactividad) + fix sistémico `[hidden]{display:none!important}`. **§32.23-.24 RE-AUDITORÍA adversarial**: "fidelidad lograda" era PREMATURA → 5 DIVERGENTES · 48 hallazgos; 13 ALTA ✅ (cifra "+18%" INVENTADA retirada · favorito muerto 8/13 · pins SERP · galería estancias · **3 de 6 los introdujo Opus "corrigiendo"** → L-29), **35 MEDIA/BAJA PENDIENTES** (ficha 8 = la más urgente, sin tocar). Crudo+síntesis en bóveda. WIP: rebuild fiel + cerrar pendientes (TODO-27). ⟦OPUS-4.8⟧ | 292 |
