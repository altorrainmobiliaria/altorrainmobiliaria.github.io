...
- **Catálogo denormalizado** (ADR §54 decisión + §56 obra-lectura): colección `indices/catalogo-{venta|arriendo|dias}` (resúmenes lean de PUBLICADAS, solo la Function escribe). LECTURA: `src/lib/domain/catalogo.ts` (tipos) · `client.ts` repo `catalogo.get(shard)` · rules `indices/{doc}` (allow-list) · ruta `src/pages/api/catalogo/[operacion].json.ts` (edge-cache purgeable). **Falta la ESCRITURA** (Function `onWrite` rebuild-total; el portal aún no tiene functions — §56.7). Ruta DORMIDA hasta cutover (SERP sigue demo).
