...
- **Cloud Functions (Node 20, `us-central1`) — el RECUENTO tiene dueño y es `05`, que lo sella contra `functions:list`**. Aquí NO se copia: se desincronizó dos veces (§180 · N14-01), la segunda con la frase «el recuento tiene dueño» escrita al lado de la copia. ⚠️ **Aquí decía «9 en CÓDIGO · 7 DESPLEGADAS» con sello del 2-ago hasta el 26-ago** — la MISMA cifra falsa que §180 corrigió en `50` el día que comparó `50` contra `05` **y no miró este nodo** (§218). Por eso este nodo ya no lleva números: nombra, y el que cuenta es `05`.
  - **DESPLEGADAS** (el RECUENTO tiene dueño y es `05`, §180): `onSolicitudStatusChanged` (email cliente) · `createManagedUserV2` / `deleteManagedUserV2` / `updateUserRoleV2`. ⚰️ **RETIRADAS el 26-ago**: `onNewSolicitud` (§199, y el 26-ago también **del código**, §217) · `onPropertyChange` + `triggerSeoRegeneration` (§217: código Y producción; con ellas se fue el uso de `GITHUB_PAT`). La primera escribía `leadScore` sobre la misma colección que `avisoLeadNuevo`, con el algoritmo sustituido; las otras dos eran inertes.
...
## §Portal (greenfield) → 🧩 hoja hija `21-MAPA-PORTAL.md`

Dónde vive cada componente, ruta, isla y script del portal **vive allá** (salió de aquí en §101:
era más de la mitad de esta neurona y es lo que crece con cada superficie nueva). Aquí se queda lo
transversal: stack, free-tier, schema, reglas, blog y SEO.
