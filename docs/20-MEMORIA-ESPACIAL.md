...
- **Componentes** (`src/components/`): **`Header.astro`** (nav sticky COMPARTIDA por todas las páginas — 3 capas: util+glass+drawer; data-driven `nav[]`; dropdowns por CSS; JS drawer+scroll; prop `active`; contacto REAL de `SITE`). Pendiente: `Footer.astro` (también compartido). Assets del logo+fotos en `public/assets/` (JPG sin optimizar → TODO WebP).
- **Páginas** (`src/pages/`): `index.astro` = **HOME** (hero neumórfico `#E6EDF2` + buscador segmentado + "Cuatro maneras"; ADR §24, parte 1 — faltan secciones) · `design-system.astro` = styleguide dev.
