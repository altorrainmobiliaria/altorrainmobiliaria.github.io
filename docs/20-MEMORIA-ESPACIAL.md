...
- **Cloud Functions (Node 20, `us-central1`) — 9 en CÓDIGO · 7 DESPLEGADAS.** Censo re-hecho el 2026-08-02 contra `functions/index.js` **y contra producción** (`verificado-vivo: 2026-08-02`, MCP Firebase). La discrepancia vieja (`05` decía 7 · `20` decía 8 y nombraba `cleanupOldLoginAttempts`, que **no existe en ninguna parte**) no era descuido: **cada nodo contaba una cosa distinta y el cerebro no tenía esa distinción**. Ahora se declara explícita.
  - **DESPLEGADAS** (el RECUENTO tiene dueño y es `05`, §180): `onSolicitudStatusChanged` (email cliente) · `onPropertyChange` (dispara GitHub Actions, **inerte**: el workflow ya no escucha → §201) · `triggerSeoRegeneration` (callable que **no invoca ningún panel**) · `createManagedUserV2` / `deleteManagedUserV2` / `updateUserRoleV2`. ⚰️ **`onNewSolicitud` RETIRADA el 26-ago** (§199): escribía `leadScore` sobre la misma colección que `avisoLeadNuevo`, con el algoritmo sustituido.
