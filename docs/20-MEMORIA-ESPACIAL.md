...
- **Catálogo denormalizado** (ADR §54 decisión + §56 obra-lectura): colección `indices/catalogo-{venta|arriendo|dias}` (resúmenes lean de PUBLICADAS, solo la Function escribe). LECTURA: `src/lib/domain/catalogo.ts` (tipos) · `client.ts` repo `catalogo.get(shard)` · rules `indices/{doc}` (allow-list) · ruta `src/pages/api/catalogo/[operacion].json.ts` (edge-cache purgeable). ESCRITURA: el NÚCLEO vive en `domain/catalogo.ts` (`construirIndices`/`propiedadAResumen`/`esPublicada`, PURO y testeable — §57); **falta la PLOMERÍA**: crear `portal/functions/` con el `onWrite` que lo invoca (el portal aún NO tiene subsistema de Functions; las legacy viven en la raíz — §57.7). Ruta DORMIDA hasta cutover (SERP sigue demo).
