...
- **Cloud Functions (Node 20, `us-central1`) — el RECUENTO tiene dueño y es `05`** (hoy: legacy 12/10, portal 20/18; `verificado-vivo: 2026-08-26`, contra `functions:list`). ⚠️ **Aquí decía «9 en CÓDIGO · 7 DESPLEGADAS» con sello del 2-ago hasta el 26-ago** — la MISMA cifra falsa que §180 corrigió en `50` el día que comparó `50` contra `05` **y no miró este nodo** (§218). Por eso este nodo ya no lleva números: nombra, y el que cuenta es `05`.
