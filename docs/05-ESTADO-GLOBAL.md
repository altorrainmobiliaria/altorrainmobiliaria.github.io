...
| **Portal (staging)** | 🔄 **OLA 1 EN RE-TRABAJO DE FIDELIDAD** (§32, TODO-27): 7 páginas construidas, **13 ALTA + FICHA (§43)+TURISMO (§45)+ESTANCIAS (§46) FIEL** ✅ (verificado build+DOM+re-audit adversarial en dev :4321); quedan **11 MEDIA/BAJA** (serp 7 · home 2 · publicar 2). Staging worker `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex) `verificado-vivo: 2026-07-12` — **redeploy pendiente (batch)**. Método: diff vs `.dc.html` + re-audit adversarial (L-29). |
