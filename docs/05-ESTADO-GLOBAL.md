...
| **Portal (staging)** | ✅ **OLA 1 FIDELIDAD COMPLETA Y LIVE (TODO-27 CERRADO §48)**: las **6 páginas FIELES** al mockup (§43-§48), **desplegadas** en `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex). `verificado-vivo: 2026-07-18` (turismo/serp/home confirmados en el worker). ⚠️ La CI `portal-ci.yml` **AUTO-DESPLIEGA cada push del portal** (`CF_DEPLOY_ENABLED` ON) — el "redeploy pendiente/07-12" era STALE (§3.3). **Go-forward**: MapLibre real · datos Firestore (TODO-22, carril Fable) · wiring forms→`solicitudes` (gate legal Ley 1581) · /arrendar = 1 card honesto hasta Firestore (Daniel decidió 07-18). |
