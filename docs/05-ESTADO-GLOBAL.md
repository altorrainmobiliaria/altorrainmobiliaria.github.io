...
| **Portal (staging)** | ✅ **OLA 1 — 13/13 y VIVA EN STAGING** `verificado-vivo: 2026-08-25` (curl contra el worker: `/seguridad`, la bóveda dentro de `/gestion` y el paso del código en `/ingresar`, todo servido y `noindex`): `altorra-portal.altorrainmobiliaria.workers.dev` sirve cero cifras inventadas. Censo de páginas → `21`. Resta: GATES DEL DUEÑO en el orden del `10`. |
