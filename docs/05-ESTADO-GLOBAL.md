...
| **Portal (staging)** | ✅ **OLA 1 FIDELIDAD COMPLETA Y LIVE** (TODO-27 cerrado, §43-§48): **23 páginas** (censo → `20`; **OLA 1 = 12/13**, §91-§95: 301 del legacy sin huecos + robots/sitemap + JSON-LD) en `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex). `verificado-vivo: 2026-08-19` (curl 200 · 135 KB · noindex,nofollow). Go-forward → TODO-30. |
...
- 📣 **HUMO cerrada ✅ · ⚠️ apagado de la campaña SIN VERIFICAR** (§90 → pelota (0) del `10`): fontanería §4b cerrada (factura con IVA OK). Cifras y benchmarks → `pauta-captacion` §10. Falta de Daniel: ¿los 3 chats eran propietarios reales? Campaña REAL gateada por matrícula + cierre de obra. 
