...
| **Portal (staging)** | ✅ **OLA 1 — 13/13 y VIVA EN STAGING** `verificado-vivo: 2026-08-26` (curl al worker: `/nosotros`, `/aliados`, `/invertir`, `/journal` y `/` → 200, y el menú desplegado YA sin el ancla fantasma `#servicios`): **cero «próximamente» en `src/pages/`** (§158-§159) · los 65 redirects servidos, `noindex` y sin el 307 a la forma con barra (§150) · cero cifras inventadas. Censo → `21`+`22`. Resta: GATES DEL DUEÑO en el orden del `10`. |
