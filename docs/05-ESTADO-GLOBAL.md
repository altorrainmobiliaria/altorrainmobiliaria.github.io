...
| **Portal (staging)** | ✅ **OLA 1 FIDELIDAD COMPLETA (TODO-27 CERRADO §48)**: las **6 páginas FIELES** al mockup (ficha/turismo/estancias/serp/home/publicar §43-§48; verificado build+DOM+re-audit adversarial en dev :4321). **Go-forward** (no fidelidad): MapLibre real · datos Firestore (TODO-22) · wiring forms→`solicitudes` · 🔸 decisión Daniel /arrendar. Staging worker `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex) `verificado-vivo: 2026-07-12` — **redeploy pendiente (batch)**. |
