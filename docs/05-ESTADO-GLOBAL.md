...
| **Portal (staging)** | ✅ **OLA 1 COMPLETA EN CÓDIGO — 13/13** (§91-§96: 301 del legacy sin huecos + robots/sitemap + JSON-LD + alertas con digest). **25 páginas** (censo → `20`) en `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex). `verificado-vivo: 2026-08-19`; §96 verificado en dev + emulador, NO en el staging desplegado. Lo que falta de la ola no es código: go/no-go + §94.6. |
