...
## 🧩 Sub-sistemas
mantenimiento LIVE ✅ · `admin.html` (consulta legacy) ✅ · sitio público viejo ⛔ RETIRADO · **CF legacy: 12 en código / 10 desplegadas** · **CF portal: 21 en código / 18 desplegadas** (contra la API, 26-ago) — las 3 sin desplegar: 2 programadas + registrarPreaviso (§233) (por qué, y las 2 legacy fuera a propósito → `50`; el tope de 3 jobs de Scheduler está en `functions/src/index.ts`). El CÓDIGO lo cuenta el gate #29; el DESPLIEGUE no se puede contar desde el repo, **por eso lleva sello** · **Auth = Identity Platform**: 2FA TOTP usable en las dos puertas, **nadie inscrito AÚN**; reglas fase 2 EN VIVO (`verificado-vivo: 2026-08-26`, §132·§137) · **modelo de datos v1** (§22 · §54 · E2E 21/21) · **mapa MapLibre §55** vivo (verificado 2026-08-20); falta la vista en foreground y **la confirma Daniel** ([[L-39]]) · D1 sellado (§23) · `gestion` (3 roles, §31) · Pendiente → **TODO-30**.
