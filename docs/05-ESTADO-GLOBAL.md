...
| **Build** | ✅ **MODO OBRA LIVE** `verificado-vivo: 2026-08-26` (curl 200 + sentinela «portal en construcción»): mantenimiento + 66 stubs→home (53 raíz · 7 `blog/` · 6 `p/`) + `.nojekyll` (L-13, §15). El SW legacy = **kill-switch** del modo obra. Sitio viejo RETIRADO; GSC preservado. |
...
| **Portal (staging)** | ✅ **OLA 1 — 13/13 y VIVA EN STAGING** `verificado-vivo: 2026-08-26` (curl al worker: `/nosotros`, `/aliados`, `/invertir`, `/journal` y `/` → 200, y el menú ya sin el ancla fantasma `#servicios`): **cero «próximamente» en `src/pages/`** (§158-§159) · 65 redirects del PORTAL (12 zonas + 53 manuales; otra cosa que los 66 stubs de arriba), `noindex`, sin el 307 a la barra (§150) · cero cifras inventadas. Censo → `21`+`22`. Resta: GATES DEL DUEÑO en el orden del `10`. |
...
## 🧩 Sub-sistemas
mantenimiento LIVE ✅ · `admin.html` (consulta legacy) ✅ · sitio público viejo ⛔ RETIRADO · **CF legacy: 12 en código / 10 desplegadas** · **CF portal: 22 en código / 19 desplegadas** (contra la API, 27-ago) — las 3 sin desplegar: 2 programadas + el aviso de estado (§235) (por qué, y las 2 legacy fuera a propósito → `50`; el tope de 3 jobs de Scheduler está en `functions/src/index.ts`). El CÓDIGO lo cuenta el gate #29; el DESPLIEGUE no se puede contar desde el repo, **por eso lleva sello** · **Auth = Identity Platform**: 2FA TOTP usable en las dos puertas, **nadie inscrito AÚN**; reglas fase 2 EN VIVO (`verificado-vivo: 2026-08-26`, §132·§137) · **modelo de datos v1** (§22 · §54 · E2E 21/21) · **mapa MapLibre §55** vivo (verificado 2026-08-20); falta la vista en foreground y **la confirma Daniel** ([[L-39]]) · D1 sellado (§23) · `gestion` (3 roles, §31) · Pendiente → **TODO-30**.
