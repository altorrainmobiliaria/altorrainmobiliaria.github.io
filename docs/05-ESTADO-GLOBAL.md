...
| **Portal (staging)** | ✅ **OLA 1 — 13/13 y VIVA EN STAGING** `verificado-vivo: 2026-08-24` (§125): `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex) sirve cero cifras inventadas, la solicitud de estancia y las 4 pantallas de gestión. ⚠️ Hasta hoy llevaba **10 ADRs de atraso** por un CI rojo que nadie miraba. Censo de páginas → `21`. Resta: GATES DEL DUEÑO en el orden del `10`. |

## ⚠️ Flags de riesgo activos
- ✅ **Matrícula de Arrendador `6636` — PUBLICADA** (Resolución 6636 del 23-jul-2026; la Alcaldía confirmó el 20-ago que el nº de matrícula ES el de la resolución y que el registro digital ya está inscrito). **Gate de Ley 820 art. 31 LEVANTADO**: ya aparece en el footer del portal y en la Política; la pauta de arriendo deja de estar bloqueada por esto. Pendiente MENOR: pedir la constancia sellada para el expediente. ⚠️ Sigue faltando la **dirección COMERCIAL** (la del acto es el domicilio del representante) y verificar en CC la matrícula del establecimiento (el acto cita 10011978, que en `43` es la de la SOCIEDAD). RNT: declarado, sin verificar. Detalle → `43 §Matrícula`.
...
## 🧩 Sub-sistemas
mantenimiento LIVE ✅ · `admin.html` (consulta legacy) ✅ · **CF legacy: 13 en código / 11 DESPLEGADAS** · **Auth = Identity Platform: 2FA TOTP disponible (nadie inscrito), clave mín 12, reglas fase 2 EN VIVO** (`verificado-vivo: 2026-08-25`, §132) · **CF portal: 9 en código** / 0. El CÓDIGO lo cuenta el gate #29; el DESPLIEGUE no se puede contar desde el repo, por eso lleva sello. Las 2 legacy fuera mandan correo solas y se quedan fuera A PROPÓSITO; las 9 del portal esperan a la fase 3 · sitio público viejo ⛔ RETIRADO · **modelo de datos v1** (§22 auditado ✅ + catálogo=doc-índice §54; E2E 21/21) · **mapa MapLibre §55** (ficha+SERP; `.pmtiles` y Worker+Range verificados vivos 2026-08-20). **Falta SOLO la vista en foreground y la confirma Daniel** — el porqué está en [[L-39]], no aquí · D1 sellado (§23, tokens.css = SSoT) · `gestion` (admin 3 roles, §31) · Pendiente → **TODO-30** (`10`).
