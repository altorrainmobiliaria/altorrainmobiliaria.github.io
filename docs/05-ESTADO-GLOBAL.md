...
| **Build** | ✅ **MODO OBRA LIVE** `verificado-vivo: 2026-08-30` (curl: 200 + sentinela + un stub 200 + **cero rastro del móvil personal en el `scripts.js` SERVIDO**, §250): mantenimiento + 66 stubs→home (53 raíz · 7 `blog/` · 6 `p/`) + `.nojekyll` (L-13, §15). El SW legacy = **kill-switch de un solo tiro** ([[L-67]]). GSC preservado. |
...
| **Portal (staging)** | ✅ **OLA 1 — 13/13 y VIVA EN STAGING** `verificado-vivo: 2026-08-31` (curl HOY: `/`, `/comprar`, `/estancias` y las 2 SSR —`/ficha` y `/alertas`, que NO están en `dist`— → 200 · ruta inventada → 404 · sirve los 13 tipos de §271 y **CERO** `alt-projcard`: §270 está en el HTML SERVIDO): 65 redirects del PORTAL (12 zonas + 53 manuales; otra cosa que los 66 stubs de arriba). ⚙️ Lo que ya bloquea un gate NO se afirma aquí (§158·§150). Censo → `21`+`22`. |
...
- 🔥 **Firebase BLAZE activo** ✅ (sin costo — el portal aún sirve datos DEMO; el porqué y la restauración → `50`).
...
## 🧩 Sub-sistemas
mantenimiento LIVE ✅ · `admin.html` (consulta legacy) ✅ · páginas viejas ⛔ RETIRADAS (sus ficheros NO: §250) · **CF legacy: 12 en código / 10 desplegadas** · **CF portal: 23 en código / 20 desplegadas** (`functions:list`, 31-ago) — faltan `alertasDigest`, `catalogoBarrido` y `avisoEstadoSolicitud`, las tres tras el gate de Resend (§283) (por qué, y las 2 legacy fuera a propósito → `50`; el tope de 3 jobs de Scheduler está en `functions/src/index.ts`). El CÓDIGO lo cuenta el gate #29; el DESPLIEGUE no se puede contar desde el repo, **por eso lleva sello** · **Auth = Identity Platform**: 2FA TOTP usable en las dos puertas, **nadie inscrito AÚN**; reglas fase 2 EN VIVO (`verificado-vivo: 2026-08-30` — 403 en 3 colecciones y en `indices/_control`; **404** en los públicos: DISCRIMINAN, §132·§137) · **modelo de datos v1** (§22 · §54 · E2E 21/21) · **mapa MapLibre §55** vivo (verificado 2026-08-20); falta la vista en foreground y **la confirma Daniel** ([[L-39]]) · D1 sellado (§23) · `gestion` (3 roles, §31) · Pendiente → **TODO-30**.
