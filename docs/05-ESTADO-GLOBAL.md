...
| **Portal (staging)** | ✅ **OLA 1 COMPLETA EN CÓDIGO — 13/13** (§91-§96: 301 del legacy sin huecos + robots/sitemap + JSON-LD + alertas con digest). páginas: el censo lo OWNea `21` (§90) — no se copia aquí en `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex). **La FICHA ya es dinámica** (§97): el portal público está completo en código. `verificado-vivo: 2026-08-19`; §96-§97 verificados en dev, no en staging. |
