...
| **Portal (staging)** | ✅ **OLA 1 COMPLETA EN CÓDIGO — 13/13**, re-verificado archivo por archivo el 2026-08-22 (§123). Lo que resta son GATES DEL DUEÑO, en el orden del `10`. páginas: el censo lo OWNea `21` (§90) — no se copia aquí en `altorra-portal.altorrainmobiliaria.workers.dev` (Astro+Workers, noindex). **La FICHA ya es dinámica** (§97): el portal público está completo en código. `verificado-vivo: 2026-08-19`; §96-§97 verificados en dev, no en staging. |
...
## 🧩 Sub-sistemas
mantenimiento LIVE ✅ · `admin.html` (consulta legacy) ✅ · **CF legacy: 11 en código** / 7 desplegadas · **CF portal: 9 en código** / 0 desplegadas (las cuenta el gate #29, no el ojo: el claim viejo decía 9 legacy con el sello fresco y eran 11). De las 4 legacy sin desplegar, 2 mandan correo solas (decisión, `20`) y 2 son el claim de staff (fase 1 del runbook); las 9 del portal esperan al cutover · sitio público viejo ⛔ RETIRADO · **modelo de datos v1** (§22 auditado ✅ + catálogo=doc-índice §54; E2E 21/21) · **mapa MapLibre §55** (ficha+SERP; `.pmtiles` y Worker+Range verificados vivos 2026-08-20). **Falta SOLO la vista en foreground y la confirma Daniel** — el porqué está en [[L-39]], no aquí · D1 sellado (§23, tokens.css = SSoT) · `gestion` (admin 3 roles, §31) · Pendiente → **TODO-30** (`10`).
